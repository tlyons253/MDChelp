...
est_accelerate<-function(OG.dat, dat.list,est.fxn){
  ...
  est.fxn(OG.dat)%>%
    rename(theta.hat=estimate)->OG.est
...
#'
#'
bootstrap_interval<-function(boot.est,OG.dat,est.fxn,alpha,multcomp=TRUE,
...
message("CI's are unadjusted and DO NOT control the family-wide error rate")
  ...
  if(type=='normal'){
      ...
      left_join(tmp,
                boot.est%>%
                  group_by(term)%>%
                  summarize(SE=sd(theta.boot)),
                by='term')%>%
      mutate(CI.lo=theta.hat+qnorm(alpha.use/2)*SE,
             CI.hi=theta.hat+qnorm(1-alpha.use/2)*SE)->out
...
if(type=='BCa'){
  ...
  MDChelp::jacknife_data(OG.dat)->jack.list


  MDChelp::est_accelerate(OG.dat,jack.list,est.fxn=FXN)->est.a
