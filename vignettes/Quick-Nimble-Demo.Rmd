...
The effective sample size for each parameter is much larger using the slice **AF_slice** sampler. There is a time difference, with the slice sampler taking `r paste0(round(slice.time,2),' seconds')` compared to the default samplers of `r paste0(round(RW.time,2),' seconds')`. It's not a large difference in this simple model, but that's `r round(slice.time/RW.time,2)` times longer. That will scale up when your models start taking longer to run. 
