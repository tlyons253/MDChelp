...
knitr::include_graphics('./images/princessbride.jpg')
```
...
The figure to the right demonstrates that. It comes from a TTE workshop at the TWS meeting in Reno in 2019, though the data are on a long-term deer survival study in northern MN.

```{r, out.width='100%'}
knitr::include_graphics('./images/hazard_surv.jpg')
```
...
Fun fact, the Weibull above is both and AFT and a PH model, and it's the only form that can be both. That's because, in the equation above, you could think of the $\alpha\log(t)$ as a baseline hazard
...
<br>
```{r, out.width='100%'}
knitr::include_graphics('./images/AFTvPH.jpg')
```
...
```{r,echo=FALSE}
 flextable::flextable(tte.dat[1:15,])%>%
...
This means the estimated daily survival from the TTE model is `r round( 1-exp(parms.nim%>%pull(mean)),digits=3)`.
...
```{r,hide=TRUE,message=FALSE,warning=FALSE, echo=TRUE, eval=FALSE}
#simulate data
library(tidyverse)
set.seed(1234)
...
# post.parms
# 
 MCMCvis::MCMCtrace(samples.time,
                     ISB=FALSE,
                     exact=TRUE,
                  params=c('gamma0','S0[1]','S0[3]','S0[25]'),
                  pdf=FALSE)
...
post.parms%>%
  ...
  rename(med=`50%`)%>%
  select(parm,med)%>%
  mutate(dsr.med=1-med,
        cumsurv.med=1-cumsum(med))->sr.haz
...
eh.long%>%
  select(nest.id,
         time,y)%>%
  complete(.,nest.id,time=seq(1,25,1),
           fill=list(y=NA))%>%
  group_by(nest.id)%>%
  pivot_wider(names_from=time,
              values_from=y)%>%
  ungroup()%>%
  mutate(nest.id=as.numeric(nest.id))%>%
  arrange(nest.id)%>%
  select(-nest.id)%>%
  as.matrix()->y
...
#throws an unintialized error still, but the log likelihood is a real number so you can ignore.
...
all.dsr<-data.frame(rbind(t(sr.haz[1:24,4]),t(dsr.logexp),t(ss.dsr), t(S)))

names(all.dsr)<-paste0('Age ',seq(1,24,1))

data.frame(type=c('Hazard','Logistic exposure','State Space','"Truth"'))%>%
  bind_cols(.,
            all.dsr)%>%
 mutate(across(2:25,~round(.x,digits=4)))%>%
  rename(Model=1)->time.tab1



   flextable::flextable(time.tab1)%>%
flextable::set_caption(flextable::as_paragraph(flextable::as_b('Table 4.'), 
                                    " Daily survival estimate comparison among methods, (time-varying survival)."),
                       align_with_table = FALSE)%>%
  flextable::theme_vanilla() %>% 
  flextable::align(align = "center", part = "all")%>%
  flextable::border_inner_h(
...
data.frame(type=c('Hazard','Logistic exposure','State Space','"Truth"'),
           c.surv=c(sr.haz[24,4],prod(dsr.logexp),prod(ss.dsr), prod(S)))%>%
 mutate(c.surv=round(c.surv,digits=3))%>%
  ...
  relocate(Parameter)->time.tab2


   flextable::flextable(time.tab2)%>%
flextable::set_caption(flextable::as_paragraph(flextable::as_b('Table 5.'), 
